#!/usr/bin/env python3
"""
Resolution Snap v2.3 — AI Outpainting Bucketer for ComfyUI
═══════════════════════════════════════════════════════════════════
...
v2.3: VRAM flush between jobs prevents GPU memory exhaustion.
...
# VRAM management — tune for your GPU
FLUSH_EVERY_N      = 1       # Flush VRAM cache after every N jobs
FULL_UNLOAD_EVERY  = 2    # Fully unload models every N jobs (heavier, slower)
...
class ComfyConnection:
    ...
    def free_memory(self, unload_models=False):
        """
        Flush ComfyUI's VRAM cache between jobs.

        unload_models=False → keep checkpoint in VRAM, flush intermediates
                              (fast, ~0.1s, frees latent/image tensors)
        unload_models=True  → fully unload everything from VRAM
                              (slower, model reloads on next job ~2-5s)
        """
        try:
            resp = requests.post(
...
def main():
    ...
    print("=" * 66)
    print("   Resolution Snap v2.3 — Alpha-Mask Outpainting Bucketer")
    print("=" * 66)
    ...
    print(f"  Mask: alpha channel  •  VRAM flush: every {FLUSH_EVERY_N} job(s)")
    ...
    try:
        for idx, filename in enumerate(files, 1):
            ...
            try:
                ...
                print(f"\n         [ERROR] {exc}")
                stats["failed"] += 1
                # Try to recover VRAM after a failure too
                if comfy:
                    comfy.free_memory(unload_models=True)
                    jobs_since_flush  = 0
                    jobs_since_unload = 0
