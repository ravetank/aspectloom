#!/usr/bin/env python3
"""
Resolution Snap v3.2 — AI Outpainting Bucketer for ComfyUI
═══════════════════════════════════════════════════════════════════════
...
v3.2: Edge-extended padding replaces flat gray — fixes solid bars.
      Model sees actual edge colors → generates matching content.
...
# ── Canvas / Mask ─────────────────────────────────────────────────────────────────
MASK_FEATHER_PX       = 16       # Feather on the inpaint mask boundary
EDGE_EXTEND_BLUR_PX   = 32      # Blur the edge-extended padding (gives model freedom)
...
def _extend_edges(canvas: Image.Image, src: Image.Image, ox: int, oy: int):
    """
    Extend source edge pixels into padding area using fast PIL ops.
    Fills padding with stretched edge strips instead of flat color.

    Before (gray bars):          After (edge-extended):
    ┌────────────────────┐       ┌────────────────────┐
    │▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓│       │←←←←←←←←←←←←←←←←←←│
    │▓▓┌──────────┐▓▓▓▓│       │←←┌──────────┐→→→→│
    │▓▓│  source  │▓▓▓▓│       │←←│  source  │→→→→│
    ...
    # Left: stretch leftmost column of source
    if ox > 0:
        left_col = src.crop((0, 0, 1, sh))
        left_fill = left_col.resize((ox, sh), Image.NEAREST)
        canvas.paste(left_fill, (0, oy))

    # Right: stretch rightmost column of source
    right_start = ox + sw
    if right_start < tw:
        right_col = src.crop((sw - 1, 0, sw, sh))
        right_fill = right_col.resize((tw - right_start, sh), Image.NEAREST)
        canvas.paste(right_fill, (right_start, oy))

    # Top: stretch the full-width row at source top (includes extended corners)
    if oy > 0:
        top_row = canvas.crop((0, oy, tw, oy + 1))
        top_fill = top_row.resize((tw, oy), Image.NEAREST)
        canvas.paste(top_fill, (0, 0))

    # Bottom: stretch the full-width row at source bottom
    bottom_start = oy + sh
    if bottom_start < th:
        bot_row = canvas.crop((0, bottom_start - 1, tw, bottom_start))
        bot_fill = bot_row.resize((tw, th - bottom_start), Image.NEAREST)
        canvas.paste(bot_fill, (0, bottom_start))


def build_canvas(img: Image.Image, tw: int, th: int) -> Image.Image:
    """
    RGBA canvas with edge-extended padding + feathered alpha mask.

    RGB layer:   source centered, edges stretched into padding, then blurred
    Alpha layer: 255 over source, 0 over padding, feathered at boundary

    The model sees plausible edge colors (not flat gray) so it generates
    content that matches the image instead of solid colored bars.
    """
    src = _fit_source(img.convert('RGB'), tw, th)
    ox = _center_offset(src.width, tw)
    oy = _center_offset(src.height, th)

    # ── Build RGB with edge-extended padding ──
    canvas_rgb = Image.new('RGB', (tw, th), (0, 0, 0))
    canvas_rgb.paste(src, (ox, oy))
    _extend_edges(canvas_rgb, src, ox, oy)

    # Blur ONLY the padding area (keeps source sharp, softens repetition)
    if EDGE_EXTEND_BLUR_PX > 0:
        blurred = canvas_rgb.filter(
            ImageFilter.GaussianBlur(radius=EDGE_EXTEND_BLUR_PX)
        )
        # Mask: white = keep sharp original, black = use blurred
    ...
    # ── Build alpha mask ──
    # 255 = opaque (keep), 0 = transparent (inpaint)
...
def process_image(
    ...
    # Build RGBA canvas with edge-extended padding
    canvas = build_canvas(img, inp_w, inp_h)

    # Debug: save canvas to inspect RGB + alpha
    if verbose:
        dbg_rgb = os.path.join(out_dir, f"_debug_{stem}_rgb.png")
        dbg_mask = os.path.join(out_dir, f"_debug_{stem}_mask.png")
        canvas.convert('RGB').save(dbg_rgb)
        canvas.getchannel('A').save(dbg_mask)
        print(f"         [debug] saved {dbg_rgb}")
        print(f"         [debug] saved {dbg_mask}")
...
def main():
    ...
    print("=" * 66)
    print("   Resolution Snap v3.2 — Edge-Extended Outpainting")
    print("=" * 66)
    ...
    print(f"  Feather:     {MASK_FEATHER_PX}px  |  Edge blur: {EDGE_EXTEND_BLUR_PX}px")
