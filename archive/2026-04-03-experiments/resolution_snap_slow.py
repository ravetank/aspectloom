#!/usr/bin/env python3
"""
Resolution Snap v3.0 — AI Outpainting Bucketer for ComfyUI
═══════════════════════════════════════════════════════════════════════
...
v3.0: Removed 1-step LoRA, embedded workflow, proper inpainting
      settings, mask feathering for seamless blending.
...
# ── Model ─────────────────────────────────────────────────────────────────────────
# Use a proper SD1.5 inpainting model (NOT the Hyper variant without its LoRA)
# Options:
#   "inpaint\\SD15\\realisticVisionV60B1_v51HyperInpaintVAE.safetensors"   ← your current
#   "inpaint\\SD15\\realisticVisionV51_v51VAE-inpainting.safetensors"      ← recommended
#   "inpaint\\SD15\\sd-v1-5-inpainting.safetensors"                        ← baseline
INPAINT_MODEL = r"inpaint\juggernautXL_versionXInpaint.safetensors"

# ── Sampler ───────────────────────────────────────────────────────────────────────
SAMPLER_STEPS     = 20           # 20-30 for quality inpainting
SAMPLER_CFG       = 7.5          # 6-9 works well; 1 = no guidance = solid borders
SAMPLER_NAME      = "dpmpp_2m_sde"
SAMPLER_SCHEDULER = "karras"
SAMPLER_DENOISE   = 1.0          # 1.0 for outpainting (full generation in masked area)
SAMPLER_SEED      = 1982
...
# ── Canvas / Mask ─────────────────────────────────────────────────────────────────
PADDING_RGB    = (128, 128, 128)  # RGB under transparent padding (invisible but safe)
MASK_FEATHER_PX = 8               # Gaussian feather on mask edge for smooth blending
...
def build_workflow(image_name: str, filename_prefix: str) -> dict:
    """
    Build the ComfyUI inpainting workflow as a Python dict.

    Flow:
      LoadImage (RGBA) → InpaintModelConditioning → KSampler → VAEDecode → SaveImage
                ↕                    ↕
           image + mask        CheckpointLoader
                               (inpainting model)
    """
    return {
        ...
        "3": {
            "inputs": {
                ...
                "clip": ["2", 1],          # CLIP from checkpoint directly
            },
        ...
        # ── Inpaint conditioning (for inpainting models) ─────────────
        "6": {
            "inputs": {
                "noise_mask": True,
                "positive": ["3", 0],      # conditioned positive
                "negative": ["4", 0],      # conditioned negative
                "vae":      ["2", 2],      # VAE from checkpoint
                "pixels":   ["8", 0],      # image from LoadImage
                "mask":     ["8", 1],      # mask from LoadImage alpha
            },
        ...
        "1": {
            "inputs": {
                ...
                "model":    ["2", 0],      # model from checkpoint directly (no LoRA)
                "positive": ["6", 0],      # from InpaintModelConditioning
                "negative": ["6", 1],      # from InpaintModelConditioning
                "latent_image": ["6", 2],  # from InpaintModelConditioning
            },
            "class_type": "KSampler",
        ...
        # ── Load image (RGBA → image + mask from alpha) ──────────────
        "8": {
            "inputs": {
                "image": image_name,       # ← replaced per job
            },
        ...
        "10": {
            "inputs": {
                "filename_prefix": filename_prefix,  # ← replaced per job
                "images": ["7", 0],
            },
...
def build_canvas(img: Image.Image, tw: int, th: int) -> Image.Image:
    ...
    Applies Gaussian feathering at the mask edge for seamless blending.
    """
    src = img.convert('RGB')
    ...
    # Feather the mask edge for smoother inpainting blending
...
def main():
    ...
    print("=" * 66)
    print("   Resolution Snap v3.0 — Inpainting Outpainter (no LoRA)")
    print("=" * 66)
    ...
    print(f"  VRAM flush: every {FLUSH_EVERY_N} job(s)")
    print(f"  Full model unload: every {FULL_UNLOAD_EVERY} job(s)\n")
    ...
    try:
        for idx, filename in enumerate(files, 1):
            ...
            # Build RGBA canvas (transparent padding = inpaint mask)
            ...
            try:
                ...
                # Build workflow with this image
                workflow = build_workflow(
                    image_name=actual_name,
                    filename_prefix=f"ResSnap/{stem}",
                )
                ...
                if jobs_since_unload >= FULL_UNLOAD_EVERY:
                    print(f"         [mem] full VRAM flush + model unload")
                    comfy.free_memory(unload_models=True)
                    jobs_since_unload = 0
                    jobs_since_flush  = 0
                elif jobs_since_flush >= FLUSH_EVERY_N:
                    comfy.free_memory(unload_models=False)
                    jobs_since_flush = 0
