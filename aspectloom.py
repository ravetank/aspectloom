#!/usr/bin/env python3
"""
Aspectloom — overlap-ring outpainting for ComfyUI.

The application:
- Default padding is transparent, not edge-extended.
- A configurable overlap ring lets the model repaint a narrow band *inside*
  the source edges so the seam can heal naturally.
- Final compositing uses a soft center-preserve mask instead of a hard paste,
  eliminating the visible inner rectangle.
- Inpaint resolution is raised and can scale up automatically when padding is large.
- Prompt wording is style-neutral by default.
"""
...
# ── Prompts ───────────────────────────────────────────────────────────────────────
POSITIVE_PROMPT = (
    "seamless outpainting, coherent continuation, matching style, matching lighting, "
    "matching perspective, natural scene continuation, highly detailed"
)
NEGATIVE_PROMPT = (
    "frame, border, bars, duplicated subject, repeated limbs, mirrored objects, text, logo, watermark, "
    "cropped, cut off, tiled texture, repeated edge fragments, low quality, blurry, jpeg artifacts"
)

# ── Canvas / Mask / Blending ─────────────────────────────────────────────────────
PADDING_MODE = "transparent"   # transparent | edge_extend
EDGE_EXTEND_BLUR_PX = 20       # only used when PADDING_MODE=edge_extend
OUTPAINT_OVERLAP_PX = 40       # editable ring inside source edges to heal seams
MASK_FEATHER_PX = 8            # softens keep-vs-edit boundary
PRESERVE_CENTER_BLEND_PX = 28  # soft composite of original center over AI result

# ── Resolution strategy ───────────────────────────────────────────────────────────
MAX_INPAINT_LONG_EDGE = 1536
HIGH_WASTE_LONG_EDGE = 2048
HIGH_WASTE_THRESHOLD = 0.22

# ── Bucket strategy ───────────────────────────────────────────────────────────────
BUCKET_STRATEGY   = "aspect"
MAX_PADDING_WARN  = 0.30
...
def _aspect_fit_bucket(width, height):
    ...
    if fit_list:
        fit_list.sort()
        _, cw, ch, waste = fit_list[0]
        return cw, ch, "pad", waste

    if down_list:
        down_list.sort()
        _, cw, ch, waste = down_list[0]
        return cw, ch, "downscale+pad", waste
...
def _make_keep_mask(tw: int, th: int, src_w: int, src_h: int, ox: int, oy: int, overlap: int) -> Image.Image:
    """255 = preserve core, 0 = editable region. Editable region includes outside area plus a narrow overlap ring."""
    alpha = Image.new("L", (tw, th), 0)

    core_w = max(1, src_w - overlap * 2)
    core_h = max(1, src_h - overlap * 2)
    core_x = ox + max(0, (src_w - core_w) // 2)
    core_y = oy + max(0, (src_h - core_h) // 2)

    alpha.paste(Image.new("L", (core_w, core_h), 255), (core_x, core_y))
    if MASK_FEATHER_PX > 0:
        alpha = alpha.filter(ImageFilter.GaussianBlur(radius=MASK_FEATHER_PX))
    return alpha


def build_canvas(img: Image.Image, tw: int, th: int) -> tuple[Image.Image, tuple[int, int, int, int]]:
    ...
    if PADDING_MODE == "edge_extend":
        ...
        if EDGE_EXTEND_BLUR_PX > 0:
            blurred = canvas_rgb.filter(ImageFilter.GaussianBlur(radius=EDGE_EXTEND_BLUR_PX))
            src_mask = Image.new("L", (tw, th), 0)
            src_mask.paste(Image.new("L", src.size, 255), (ox, oy))
            canvas_rgb = Image.composite(canvas_rgb, blurred, src_mask)

    alpha = _make_keep_mask(tw, th, src.width, src.height, ox, oy, OUTPAINT_OVERLAP_PX)
    canvas_rgb.putalpha(alpha)
    return canvas_rgb, (ox, oy, src.width, src.height)


def compute_inpaint_size(tw: int, th: int, waste: float) -> tuple[int, int]:
    long_edge_cap = HIGH_WASTE_LONG_EDGE if waste >= HIGH_WASTE_THRESHOLD else MAX_INPAINT_LONG_EDGE
    long_edge = max(tw, th)
...
def composite_result(ai_result: Image.Image, original: Image.Image, full_tw: int, full_th: int) -> Image.Image:
    """Blend the untouched center back softly instead of hard-pasting the whole source rectangle."""
    ...
    mask = _make_keep_mask(full_tw, full_th, src.width, src.height, ox, oy, OUTPAINT_OVERLAP_PX)
    if PRESERVE_CENTER_BLEND_PX > 0:
        mask = mask.filter(ImageFilter.GaussianBlur(radius=PRESERVE_CENTER_BLEND_PX))

    base = ai_rgb.copy()
    overlay = Image.new("RGB", (full_tw, full_th), (0, 0, 0))
    overlay.paste(src, (ox, oy))
...
def print_bucket_table():
    ...
    # Keep terminal output ASCII-safe for Windows consoles using legacy code pages.
    print(f"  {'-'*14}   {'-'*20}  {'-'*8}  {'-'*12}")
    for bw, bh in RESOLUTION_BUCKETS:
        if (bw, bh) in printed:
            continue
...
def process_image(
    ...
    inp_w, inp_h = compute_inpaint_size(tw, th, waste)
    capped = (inp_w != _snap8(tw) or inp_h != _snap8(th))
    if capped:
        print(f"         inpaint at {inp_w}x{inp_h} -> upscale to {tw}x{th}")

    canvas, _ = build_canvas(img, inp_w, inp_h)
    if verbose or save_debug:
        ...
        if verbose:
            print("         [debug] saved rgb + mask to _debug/")
...
def main():
    global BUCKET_STRATEGY, PADDING_MODE

    ap = argparse.ArgumentParser(
        description=(
            "Standardize a folder of images by outpainting each image to the "
            "closest conventional resolution with ComfyUI."
        )
    )
    ...
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="show bucket choices without creating files or contacting ComfyUI",
    )
    ap.add_argument(
        "--list-buckets",
        action="store_true",
        help="print every available target resolution and exit",
    )
    ap.add_argument(
        "--verbose",
        "-v",
        action="store_true",
        help="print WebSocket events and save diagnostic images",
    )
    ap.add_argument(
        "--save-debug",
        action="store_true",
        help="save the generated RGB canvas and mask in OUTPUT/_debug",
    )
    ap.add_argument(
        "--overwrite",
        action="store_true",
        help="replace output files that already exist (default: skip them)",
    )
    ap.add_argument(
        "--input-dir",
        help="folder containing source images (not scanned recursively)",
    )
    ap.add_argument(
        "--output-dir",
        help="destination folder (default: INPUT/aspectloom-output)",
    )
    ap.add_argument(
        "--comfy-url",
        default=COMFY_URL,
        help=f"ComfyUI server URL (default: {COMFY_URL})",
    )
    ap.add_argument(
        "--model",
        default=INPAINT_MODEL,
        help=f"ComfyUI checkpoint path (default: {INPAINT_MODEL})",
    )
    ap.add_argument(
        "--bucket-strategy",
        choices=("aspect", "first_fit"),
        default=BUCKET_STRATEGY,
        help="aspect minimizes ratio change; first_fit minimizes canvas area",
    )
    ap.add_argument(
        "--padding-mode",
        choices=("transparent", "edge_extend"),
        default=PADDING_MODE,
        help="initial pixels supplied outside the source image",
    )
    args = ap.parse_args()
    ...
    if not args.input_dir:
        ap.error("--input-dir is required unless --list-buckets is used")

    input_dir = os.path.abspath(os.path.expanduser(args.input_dir))
    output_dir = os.path.abspath(
        os.path.expanduser(
            args.output_dir or os.path.join(input_dir, "aspectloom-output")
        )
    )
    ...
    print("=" * 66)
    print(f"   {APP_NAME} v{__version__} - ComfyUI Outpainting")
    print("=" * 66)
    ...
    if not files:
        print(f"\n  No supported images in {input_dir}")
        return
    ...
    print(f"  Max inpaint:   {MAX_INPAINT_LONG_EDGE}px  |  High-waste: {HIGH_WASTE_LONG_EDGE}px")
    print(f"  Overlap ring:  {OUTPAINT_OVERLAP_PX}px  |  Mask feather: {MASK_FEATHER_PX}px")
    print(f"  Center blend:  {PRESERVE_CENTER_BLEND_PX}px")
        ...
        for idx, filename in enumerate(files, 1):
            ...
            if action == "exact":
                ...
                if (
                    ...
                    print(f"  - {tag} {filename} - output exists, skipping")
                    stats["skipped"] += 1
                    continue
            ...
            if os.path.exists(out_path) and not args.overwrite:
                print("         output exists, skipping (use --overwrite to replace)")
                stats["skipped"] += 1
                continue
    ...
    print("   SUMMARY")
    print("=" * 66)
    filled_label = "Would AI-fill:" if args.dry_run else "AI-filled:"
    exact_label = "Already standard:"
    print(f"   {filled_label:<18} {stats['filled']}")
    print(f"   {exact_label:<18} {stats['exact']}")
    print(f"   {'Skipped existing:':<18} {stats['skipped']}")
    print(f"   {'Failed:':<18} {stats['failed']}")
    print(
        f"   {'Total time:':<18} {total_time:.0f}s  "
